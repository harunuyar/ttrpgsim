...
public class WeaponAttackAction : AttackRollDamageAction, IWeaponAttackAction
{
    ...
    public WeaponAttackAction(ActionDurationType actionDurationType, DamageTypeModel damageType, IEquipmentInstance weapon, EAttackHandType attackHandType)
        : base("Weapon Attack", actionDurationType, [], GetActionRange(weapon), TargetingType.SingleTarget, damageType, GetDicePool(weapon, attackHandType))
    {
        Weapon = weapon;
    ...
    private static ActionRange GetActionRange(IEquipmentInstance weapon)
    {
        var normalRangeDistance = Distance.OfFeet(weapon.EquipmentModel.Range?.Normal ?? 0);
        var longRangeDistance = weapon.EquipmentModel.Range?.Long is not null ? Distance.OfFeet(weapon.EquipmentModel.Range.Long.Value) : null;

        return weapon.EquipmentModel.WeaponRange == EWeaponRange.Melee
                ? ActionRange.Melee(normalRangeDistance, longRangeDistance)
                : ActionRange.Ranged(normalRangeDistance, longRangeDistance);
    }

    private static DicePool GetDicePool(IEquipmentInstance weapon, EAttackHandType attackHandType)
    {
        return DicePool.Parse(attackHandType == EAttackHandType.Versatile
            ? weapon.EquipmentModel.TwoHandedDamage?.DamageDice
            : weapon.EquipmentModel.Damage?.DamageDice);
    }
}
