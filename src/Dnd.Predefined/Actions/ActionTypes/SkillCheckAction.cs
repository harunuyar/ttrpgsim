...
public class SkillCheckAction : EventAction, ISkillCheckAction
{
    public SkillCheckAction(SkillModel skill)
        : base($"{skill.Name} Check", ActionDurationType.Action, [])
    {
        Skill = skill;
    }
    public SkillModel Skill { get; }

    public ESuccessRollType SuccessRollType => ESuccessRollType.Skill;

    public override Task<IEvent> CreateEvent(IGameActor actor)
    {
        return Task.FromResult<IEvent>(new RollSuccessEvent(Name, actor, this, null, null));
    }
